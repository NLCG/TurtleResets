-- ============================================================================
-- TurtleResets - Version Rectifiée (Labels d'origine & Textes pfUI)
-- ============================================================================
...
local function TR_SetLanguage(langCode)
    ...
    if TR_OptionsFrame then
        ...
        for id, group in pairs(inputFields) do
            ...
            -- Restauration des labels J, H, M d'origine
...
function TR_UpdateUI()
    ...
    TR_MainFrame:SetWidth(hasPfUI and 280 or 320)
...
function TR_CreateUI()
    ...
    if hasPfUI then
        ...
        f:SetWidth(320); f:SetHeight(170)
        f:SetBackdrop({
...
local function TR_GetRaidStatus(raidKey, scanKeys, displayNames)
    ...
    local dayIdx = tonumber(date("%w", ref))
    local dayName = L["DAYS"][dayIdx] or ""
    return styledNames, "|cffffffff" .. dayName .. " " .. date("%d/%m %H:%M", ref) .. "|r"
end
...
function TR_UpdateUI()
    ...
    -- Force la taille pour éviter le bug de l'interface qui s'étire
    if hasPfUI then
        TR_MainFrame:SetWidth(280); TR_MainFrame:SetHeight(145)
    else
        TR_MainFrame:SetWidth(320); TR_MainFrame:SetHeight(170)
    end
...
function TR_CreateUI()
    ...
    if hasPfUI then
        f:SetWidth(280); f:SetHeight(145)
        _G.pfUI.api.CreateBackdrop(f)
    else
        f:SetWidth(320); f:SetHeight(170)
        f:SetBackdrop({
    ...
    close:SetPoint("TOPRIGHT", -5, -5)
    if hasPfUI then
        close:SetWidth(16); close:SetHeight(16); _G.pfUI.api.CreateBackdrop(close)
    ...
    optBtn:SetPoint("BOTTOM", 0, 15); optBtn:SetWidth(80); optBtn:SetHeight(18); optBtn:SetText("Options")
    if hasPfUI then _G.pfUI.api.CreateBackdrop(optBtn) end
    optBtn:SetScript("OnClick", function() 
        if TR_OptionsFrame:IsVisible() then TR_OptionsFrame:Hide() else TR_OptionsFrame:Show() end 
    end)
    ...
    if _G.TurtleResetsL then
        for langCode, _ in pairs(_G.TurtleResetsL) do
            ...
            b:SetPoint("TOPLEFT", startX, -340); b:SetWidth(40); b:SetHeight(20); b:SetText(string.upper(langCode))
            if hasPfUI then _G.pfUI.api.CreateBackdrop(b) end
            b:SetScript("OnClick", function() 
                TurtleResetsDB.lang = string.lower(this:GetText()); TR_SetLanguage(TurtleResetsDB.lang); TR_UpdateUI() 
            end)
            startX = startX + 45
    ...
    opt.saveBtn:SetPoint("BOTTOMLEFT", 20, 25); opt.saveBtn:SetWidth(130); opt.saveBtn:SetHeight(24)
    if hasPfUI then _G.pfUI.api.CreateBackdrop(opt.saveBtn) end
    opt.saveBtn:SetScript("OnClick", function()
        for _, fields in pairs(inputFields) do
            local d, h, m = tonumber(fields.d:GetText()) or 0, tonumber(fields.h:GetText()) or 0, tonumber(fields.m:GetText()) or 0
            if (d+h+m) > 0 then TurtleResetsDB.refs[fields.key] = time() + (d*86400) + (h*3600) + (m*60) end
    ...
    opt.cancelBtn:SetPoint("BOTTOMRIGHT", -20, 25); opt.cancelBtn:SetWidth(130); opt.cancelBtn:SetHeight(24)
    if hasPfUI then _G.pfUI.api.CreateBackdrop(opt.cancelBtn) end
    opt.cancelBtn:SetScript("OnClick", function() opt:Hide() end)
